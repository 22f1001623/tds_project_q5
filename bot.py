...
2. Use the run_python tool to fetch data and compute — do not guess numeric results you can compute.
3. Match the requested JSON shape for "answer" EXACTLY as spelling out in the question.
4. When you are ready to answer, reply with ONLY that JSON object — no prose, no markdown fences.
"""
...
def solve(chat_id: int, question: str) -> str:
    ...
    if obj is None:
        ...
        try:
            ...
            obj = {"answer": (final_text or "unable to determine").strip()}
...
# Single global instantiation config
